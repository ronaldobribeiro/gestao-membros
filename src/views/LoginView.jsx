...
export default function LoginView() {
  const { loginError, loggingIn, handleGoogleLogin } = useAuth();
  return (
    <div className="login-wrap">
      <div className="login-brand-panel">
        <img className="login-logo-svg" src={logoFull} alt="Igreja Presbiteriana Independente Jardim Cavallari" />
      </div>
      <div className="login-form-panel">
        <div className="login-card">
          <div className="login-eyebrow">Gestão de Membros</div>
          <h1 className="login-title">Entrar</h1>
          <p className="login-sub">Acesso restrito à liderança e secretaria.</p>
          {loginError ? <div className="error-box">{loginError}</div> : null}
          <button type="button" className="btn-google-login" disabled={loggingIn} onClick={handleGoogleLogin}>
            <GoogleIcon />
            <span>{loggingIn ? "Redirecionando…" : "Continuar com Google"}</span>
          </button>
        </div>
      </div>
    </div>
